...
> 検証範囲の訂正（2026-09-23）: 以下のチェック済み項目にはAPIや途中実装も含まれており、Phase 3全体の完了を証明するものではありません。地図は移動停止のみ、ビーコンはタイマーのみで、非常灯・絶縁ケース・オンラインのデバフ統合などに未実装があります。詳細は phase-04.md の残項目を参照してください。
