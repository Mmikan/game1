...
namespace Game1.Enemies
{
    public sealed class ClientRescueScenario : MonoBehaviour
    {
        ...
        private IEnumerator Start()
        {
            ...
            host.transform.position = client.transform.position + Vector3.forward;
            host.SetSupportServerRpc(client.NetworkObjectId, true);
            Check(!client.IsSupportedByAlly(), "support_front_rejected");
            host.transform.position = client.transform.position + Vector3.back;
            host.SetSupportServerRpc(client.NetworkObjectId, true);
            yield return null;
            Check(client.IsSupportedByAlly(), "support_started");
            Vector3 beforeFollow = host.transform.position;
            client.transform.position += Vector3.forward * 0.4f;
            yield return new WaitForSeconds(0.3f);
            Check(host.transform.position.z > beforeFollow.z + 0.2f && client.IsSupportedByAlly(), "supporter_follows_target");
            Check(!client.TryStartSupportOnServer(host) && client.SupportingClientId.Value == CoopAuthorityRules.NoClient, "support_cycle_rejected");
            host.SetSupportServerRpc(client.NetworkObjectId, false);
            Check(!client.IsSupportedByAlly(), "support_explicit_cancel");
            host.SetSupportServerRpc(client.NetworkObjectId, true);
